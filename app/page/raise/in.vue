<template>
  <Panel :class="$style.panel" title="In货推荐">
    <section :class="$style.content">
      <div :class="$style.item" v-for="item in items" :key="item.catoryName">
        <img :src="item.catory" :alt="item.catoryName">
        <section :class="$style.list">
          <div :class="$style.product" v-for="product in item.productList" :key="product.title">
            <img :src="product.src" :alt="product.title">
            <p :class="$style.title">{{product.title}}</p>
            <p :class="$style.price"><em>{{product.price}}</em>起</p>
            <article>
              <div :class="$style.wrapper">
                <div :class="$style.inner" :style="{width: product.rate+'%'}"></div>
              </div> 
              <p>{{product.rate}}%</p>
            </article>
            <label v-if="product.isShow">精选</label>
          </div>
        </section>
      </div>
    </section>
